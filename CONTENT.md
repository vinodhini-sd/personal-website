# Website Content Guide

Fill in the sections below with your content. I'll use this to populate your website.

---

## 1. HERO SECTION (Homepage)

**Your Name:**
```
Vino Duraisamy
```

**Title/Tagline:**
```
DEVELOPER ADVOCATE @ SNOWFLAKE
```

**Animated Words (3 words that rotate):**
```
1. AI
2. Open source
3. Data
```

**Hero Description (2-3 sentences):**
```
With a decade of experience building enterprise-scale data & AI products at retail, legal and big tech companies, Vino helps developers build ideas into production-ready reality.
```

**Code Block Content:**
- Name: Vino Duraisamy
- Experience Years: 10
- Location: San Francisco
- Expertise List (6-7 items):
  1. AI Engineering
  2. Machine Learning
  3. Data Engineering
  4. Public Speaking
  5. Technical Writing
  6. Go-to-Market Strategy

---

## 2. EXPERIENCE SECTION

### Job 1 (Most Recent)
- **Job Title:**
- **Company:**
- **Date Range:**
- **Location:**
- **Description (2-3 sentences):**
- **Key Skills (5-8 tags):**

### Job 2
- **Job Title:**
- **Company:**
- **Date Range:**
- **Location:**
- **Description (2-3 sentences):**
- **Key Skills (5-8 tags):**

### Job 3
- **Job Title:**
- **Company:**
- **Date Range:**
- **Location:**
- **Description (2-3 sentences):**
- **Key Skills (5-8 tags):**

### Job 4
- **Job Title:**
- **Company:**
- **Date Range:**
- **Location:**
- **Description (2-3 sentences):**
- **Key Skills (5-8 tags):**

_Add more jobs as needed_

---

## 3. SKILLS SECTION

### Technical Skills Category 1
**Category Name:** (e.g., "Programming Languages")
**Skills:** (list them separated by commas)
```
Python, JavaScript, SQL, etc.
```

### Technical Skills Category 2
**Category Name:** (e.g., "Frameworks & Tools")
**Skills:**
```
```

### Technical Skills Category 3
**Category Name:** (e.g., "Cloud & Data Platforms")
**Skills:**
```
```

### Technical Skills Category 4
**Category Name:** (e.g., "AI/ML Technologies")
**Skills:**
```
```

_Add more categories as needed_

---

## 4. EDUCATION SECTION

### Degree 1
- **Degree:** Master of Science - Business Analytics
- **Institution:** W. P. Carey School of Business – Arizona State University, Phoenix, Arizona
- **Date Range:** 
- **Honors/Notes:** Summa Cum Laude

### Degree 2
- **Degree:** Bachelor of Engineering - Electronics and Communications
- **Institution:** PSG College of Technology, Coimbatore, India
- **Date Range:**
- **Honors/Notes:**

_Add more degrees/certifications as needed_

---

## 5. SPEAKING SECTION

**Section Introduction (1-2 sentences):**
```
[Describe your speaking experience and focus areas]
```

### Talk/Event 1
- **Event Name:**
- **Talk Title:**
- **Date:**
- **Location:**
- **Description (1-2 sentences):**
- **Topics Covered (3-5 tags):**

### Talk/Event 2
- **Event Name:**
- **Talk Title:**
- **Date:**
- **Location:**
- **Description (1-2 sentences):**
- **Topics Covered (3-5 tags):**

### Talk/Event 3
- **Event Name:**
- **Talk Title:**
- **Date:**
- **Location:**
- **Description (1-2 sentences):**
- **Topics Covered (3-5 tags):**

_Add more talks as needed_

---

## 6. WRITING SECTION

**Section Introduction (1-2 sentences):**
```
[Describe your writing focus and publications]
```

### Article 1
- **Title:**
- **Publication/Platform:**
- **Date:**
- **Link:**
- **Description (1-2 sentences):**
- **Topics (3-5 tags):**

### Article 2
- **Title:**
- **Publication/Platform:**
- **Date:**
- **Link:**
- **Description (1-2 sentences):**
- **Topics (3-5 tags):**

### Article 3
- **Title:**
- **Publication/Platform:**
- **Date:**
- **Link:**
- **Description (1-2 sentences):**
- **Topics (3-5 tags):**

_Add more articles as needed_

---

## 7. TEACHING SECTION

**Section Introduction (1-2 sentences):**
```
[Describe your teaching experience and philosophy]
```

### Course/Workshop 1
- **Course Name:**
- **Institution/Platform:**
- **Date/Duration:**
- **Description (2-3 sentences):**
- **Topics Covered (3-5 tags):**

### Course/Workshop 2
- **Course Name:**
- **Institution/Platform:**
- **Date/Duration:**
- **Description (2-3 sentences):**
- **Topics Covered (3-5 tags):**

### Course/Workshop 3
- **Course Name:**
- **Institution/Platform:**
- **Date/Duration:**
- **Description (2-3 sentences):**
- **Topics Covered (3-5 tags):**

_Add more courses/workshops as needed_

---

## 8. CONTACT SECTION

**Section Heading:**
```
Let's Build Something Amazing
```

**Section Description:**
```
[1-2 sentences about how people can reach you or work with you]
```

**Contact Cards:**

### LinkedIn
- **Heading:** Connect
- **Description:** Professional Network
- **Link:** https://www.linkedin.com/in/vinodhini-sd/

### YouTube
- **Heading:** Watch
- **Description:** Video Content & Tutorials
- **Link:** https://www.youtube.com/@vino_duraisamy/playlists

### Sessionize
- **Heading:** Speaker Profile
- **Description:** Talks & Presentations
- **Link:** https://sessionize.com/vino-duraisamy

---

## 9. BIO PAGE

**Short Bio (50 words) - For quick introductions:**
```
Vino Duraisamy is a Developer Advocate at Snowflake with a decade of experience building enterprise-scale data and AI products. She specializes in helping developers transform ambitious ideas into production-ready solutions through technical advocacy, education, and open-source contributions.
```

**Long Bio (150 words) - For conference programs and detailed bios:**
```
Vino Duraisamy is a Developer Advocate at Snowflake, where she empowers developers to build scalable data and AI solutions. With over 10 years of experience across retail, legal, and big tech companies, Vino has architected enterprise-grade data platforms and ML systems that serve millions of users.

As a passionate advocate for developers, Vino bridges the gap between complex technology and practical implementation through speaking engagements, technical writing, and open-source contributions. She specializes in data engineering, machine learning operations, and developer experience.

Vino holds a Master's degree in Business Analytics from Arizona State University (Summa Cum Laude) and a Bachelor's in Electronics and Communications from PSG College of Technology. She is a frequent speaker at conferences and meetups, sharing insights on AI engineering, data platforms, and building developer-first products.
```

**Files Needed:**
- CV/Resume: Upload your PDF to `assets/cv/Vino_Duraisamy_CV.pdf`
- Headshot: Your current headshot is at `assets/images/vino.JPG` (can be replaced with higher resolution if needed)

---

## NOTES:
- Fill in all sections with your content
- Keep descriptions concise and impactful
- Use consistent date formats (e.g., "Jan 2020 - Present" or "2020 - Present")
- For skills/topics, use short, searchable keywords
- Leave any section blank if you don't want it on the website yet
- Once you fill this out, I'll update the website with all your content!

